...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item: items) {
            ...
            work.clear(); // Now the vector is empty (all keys are 'unset').
        ...
        for (VectorEntry e: docFreq) {
            docFreq.set(e, logN - Math.log(e.getValue()));
        }
        ...
        for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
            ...
            for(VectorEntry e: tv) {
                tv.set(e, e.getValue() * docFreq.get(e.getKey())); //e.getValue() = term frequency,
            }                                                    //docFreq.get(e.getKey()) = inverse document frequency
